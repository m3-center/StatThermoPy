...
def test_psi4_thermodynamics(tmpdir):
    ...
    ZPVE, VibTemp_all, Evib_sum, Svib_sum, CVib_sum = vibrational_contribution(temperature=298.15, vibrations= \
        frequencies, scaling_factors=[1.0]*len(frequencies))

    Cv = vibrational_heat_capacity(CElec=CElec, CTrans=CTrans, CRot=CRot, CVib_sum=CVib_sum)
    ...
    final_thermo = final_themro_energies(Eelec=-354.2051381816264666, ZPVE=ZPVE, Ucorr=Ucorr, Hcorr=Hcorr,
                                         Gcorr=Gcorr)
    ...
    output = [round(ETrans, 3), round(STrans, 6), round(CTrans, 6),
              round(ERot, 3), round(SRot, 6), round(CRot, 6),
              round(ZPVE, 3),
              VibTemp_all,
              round(Evib_sum, 3), round(Svib_sum, 6), round(CVib_sum, 6),
              round(Cv, 6), round(S, 6), round(Ucorr, 3), round(Hcorr, 3), round(Gcorr, 3),
              final_thermo_list]
    ...
    target_VibTemp = [round(vib, 3) for vib in target_VibTemp]

    target = [0.889, 0.040457, 0.002981,
              0.889, 0.030049, 0.002981,
              174.217,
              target_VibTemp,
              179.900, 0.037539, 0.037223,
              0.043185, 0.108045, 181.677, 182.270, 150.056,
              [-353.92750650, -353.91561704, -353.91467285, -353.96600869]
             ]
...
def test_psi_thermodynamics_2(tmpdir):
    ...
    Hcorr = correction_energy_H(temperature=298.15, Ucorr=Ucorr)
    Gcorr = correction_energy_G(temperature=298.15, Hcorr=Hcorr, entropy=S)
    final_thermo = final_themro_energies(Eelec=-154.0756679596933054, ZPVE=ZPVE, Ucorr=Ucorr, Hcorr=Hcorr,
                                         Gcorr=Gcorr)
    # Rounding to remove the small discrepancies
    VibTemp_all = [round(vib, 3) for vib in VibTemp_all]
    final_thermo_list = [round(value, 8) for value in final_thermo]
    ...
    target = [0.889, 0.037406, 0.002981,
              ...
              [-153.98964190, -153.98551030, -153.98456612, -154.01489634]
             ]
...
def test_gammes_thermodynamics(tmpdir):
    ...
    Hcorr = correction_energy_H(temperature=298.15, Ucorr=Ucorr)
    Gcorr = correction_energy_G(temperature=298.15, Hcorr=Hcorr, entropy=S)
    final_thermo = final_themro_energies(Eelec=-154.0757448407, ZPVE=ZPVE, Ucorr=Ucorr, Hcorr=Hcorr,
                                         Gcorr=Gcorr)
    # Rounding to remove the small discrepancies
    VibTemp_all = [round(vib, 3) for vib in VibTemp_all]
    final_thermo_list = [round(value, 8) for value in final_thermo]
